...
#ifndef WEBAUTH_GTK_REQUEST_H
#define WEBAUTH_GTK_REQUEST_H
 ...
 *  is this. The backend exports one of these at the @handle path the frontend
 *  passed to Start, on the BACKEND's own bus name, and connects "handle-close"
 *  to the transaction. When the application calls Close() on the public Request,
 *  the frontend calls Close() on this one, and the window goes away.
 *
...
#define WEBAUTH_IMPL_REQUEST_INTERFACE "io.github.sjtrotter.impl.portal.Request"

typedef struct WebAuthImplRequest WebAuthImplRequest;

/** @sender is the frontend's unique name; @app_id is the application identity
 *  the frontend derived, carried here only so the chrome and any certificate
 *  dialog can name it; @id is the object path the frontend chose. */
